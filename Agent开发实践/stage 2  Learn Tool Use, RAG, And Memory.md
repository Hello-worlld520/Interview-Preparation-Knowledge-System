...
**python的库就是别人写好的、可以重复使用的代码集合**

需要用的时候安装再导入就行了

### python中的with

`with` 语句的核心是**上下文管理协议**，它保证：无论代码块是否发生异常，资源都会被正确释放。
...
```python
with DDGS() as ddgs:
    results = ddgs.text("Python 教程")
# 退出后自动清理，不需要手动管理
# 创建DDGS()当作ddgs，倒不明白看下面，下面那个清晰
```
...
* ## 工具
  ...
  1. **工具说明书**（tools 里的定义）——告诉模型「有这个工具、参数叫什么」
  2. **工具实现函数**——真正发请求、解析结果
...
* ## 搜索工具
  ...
  | 方式                                      | 优点               | 缺点                     | 适合            |
  | ----------------------------------------- | ------------------ | ------------------------ | --------------- |
  | DuckDuckGo 库                             | 免费、免 Key、简单 | 有时不稳定、结果质量一般 | 学习、本地 Demo |
  | 搜索引擎官方 API（Bing/Google/Serper 等） | 稳定、质量高       | 要 Key、可能收费         | 正式项目        |
  | 自己爬搜索页                              | 不依赖官方 API     | 易封、解析脆、有合规风险 | 不推荐          |
  ...
  tools = [
      {
          ...
          "function": {
              "name": "web_search",
              "description": "在互联网上搜索公开信息，返回若干条结果（标题、链接、摘要）。当需要最新新闻、事实、外部资料、版本更新说明时使用。不要用于闲聊，也不要用于已提供的公司内部制度问题。",
              "parameters": {
                  "type": "object",
                  "properties": {
                      "query": {
                          "type": "string",
                          "description": "搜索关键词。尽量具体、可检索，例如：'Python 3.13 新特性'，而不是 '帮我查一下那个东西'。"
                      },
                      "max_results": {
                          "type": "integer",
                          "description": "返回结果条数，默认 5，建议 3 到 8。"
                      }
                  },
                  "required": ["query"]
              }
  ...
  * "required": ["query"]
  ...
  就是用了一个DDGS（）外部函数，剩下的都是在处理返回的结果，就是说有能用的函数就不要造轮子
...
* ## 数据库工具
  ...
  cur.execute("SELECT * FROM users")  # 执行查询
...
路由表是系统架构中的一种映射机制，它将请求标识符（如函数名、URL路径、消息类型）与对应的处理逻辑（处理器、控制器、回调函数）进行绑定，从而实现请求的动态分发与解耦
